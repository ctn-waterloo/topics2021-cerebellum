...
def build_test_network(input_descr,
    ...
    with nengo.Network(seed=rng.randint(1 << 31)) as model:
        ...
        if input_type == "white_noise":
            _, high = input_descr
            nd_in = nengo.Node(
                nengo.processes.WhiteSignal(high=high, period=T, rms=0.5))
        elif input_type == "pulse":
            theta = net_granule_golgi.theta
            _, t_off, t_on = input_descr
            t_off, t_on = theta * t_off, theta * t_on
            nd_in = nengo.Node(lambda t: 1.0
                               if (t % (t_on + t_off)) > t_off else 0.0)
...
def build_and_run_test_network(input_descr,
    ...
    if record_weights:
        ...
        fn = "out/benchmark/weights/weights_" + m.hexdigest()[:8] + ".h5"
        ...
        with h5py.File(fn, 'w') as f:
            ...
            def store_weights_for_conn(conn_name):
                ...
                for key, value in W.items():
                    dset_W = f.create_dataset("weights_" + conn_name + "_" +
                                              repr(key),
                                              data=value,
                                              chunks=True,
                                              compression="lzf")
    ...
    if probe_spatial_data:
        ...
        spatial_data = {
            ...
            x_granule,
            "ps":
            cvt_go_gr.get_probabilities(golgi.n_neurons, granule.n_neurons,
                                        golgi, granule, sim.data),
            "W":
            sim.model.params[net.conn_pcn_go_to_gr].weights,
        }
        res[5] = spatial_data
...
def _run_benchmark_callback(p):
    ...
    with warnings.catch_warnings():  # Silence the decoder cache UserWarning
        ...
        ts, xs, As, theta = build_and_run_test_network(rng=rng, **kwargs)
...
def run_benchmark(sweep,
    ...
    # the parameters.
    filename = ("out/benchmark/" + filename_prefix + "_" +
    ...
    with h5py.File(filename, "w") as f:
        ...
        with EnvGuard({
            ...
            with ctx.Pool(concurrency) as pool:
                for idcs, err in tqdm(pool.imap_unordered(
                        _run_benchmark_callback, params),
                                      total=len(params)):
